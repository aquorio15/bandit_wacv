...
def valid(args, model, writer, test_loader, UCB_Count_Score, epoch, global_step):
    # Validation!
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        with torch.no_grad():
            # logits = model(x)[0]
            logits = model(
                x=x, counter=epoch, UCB_Count_Score=UCB_Count_Score, ucb=True
            )[0]
...
def train(args, model):
    ...
    losses = AverageMeter()
    global_step, best_acc, step1 = 0, 0, 0
    block_size = (args.img_size * args.img_size) // (16 * 16) + 1
    ...
    for epoch in range(args.num_steps):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                losses.update(loss.mean().item() * args.gradient_accumulation_steps)
                if args.fp16:
...
def main():
    ...
    parser.add_argument(
        "--eval_batch_size", default=256, type=int, help="Total batch size for eval."
    )
    parser.add_argument(
        "--eval_every",
        default=100,
