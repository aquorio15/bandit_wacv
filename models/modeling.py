...
class Attention(nn.Module):
    ...
    def UCB_Score(self, Attention_Score, beta, iter, v, Count_score):
        score_sum = torch.sum(Count_score, dim=0)
        num = torch.tensor(np.log(iter), dtype=torch.float32)
        UCB_score = Attention_Score + beta * torch.sqrt(num / score_sum)
        _, Max_Indiices = torch.topk(UCB_score, dim=3, k=10)
        one_hot_vector = F.one_hot(Max_Indiices, num_classes=v.shape[-2])
    ...
    def forward(self, hidden_states, counter, UCB_Count_Score, ucb):
        ...
        weights = att if self.vis else None
        count_score = 0
        if counter > 500:
            if ucb:
                Count_Score = UCB_Count_Score
                attention_prob, Count_Score = self.UCB_Score(
                    att,
            ...
            attention_output = self.out(context_layer)
            attention_output = self.proj_dropout(attention_output)
        return attention_output, weights, count_score
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, counter, UCB_Count_Score, ucb, labels=None):
        ...
        if labels is not None:
            ...
            return loss, Count_Score
        else:
            return logits, attn_weights, Count_Score
