...
from models.modelling_baseline import VisionTransformer, CONFIGS
from models.scheduler import WarmupLinearSchedule, WarmupCosineSchedule
from models.data_utils import get_loader
from models.dist_util import get_world_size
...
def setup(args):
    ...
    num_classes = 8 if args.dataset == "communication" else 100
    # num_classes = 200
...
def valid(args, model, writer, test_loader, global_step, class2num):
    # Validation!
    ...
    for step, batch in enumerate(epoch_iterator):
        x, y = batch['input'], batch['snr']
        x = x.float().cuda()
        mod_ = torch.Tensor([class2num[jj] for jj in batch['modtype']]).long()
...
def train(args, model):
    ...
    trainset, testset = get_loader(args)
    ...
    class2num = testset.class2num()
    curriculum = 0
    
    for epoch in range(args.num_steps):
        if epoch == args.switch_epochs[curriculum]:
            logger.info("----[Curriculum #%s] SNR=%s:2:28dB----" % (curriculum, 28-2*curriculum))
            ...
            if not curriculum == len(args.switch_epochs)-1:
                curriculum += 1
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.eval_every == 0 and args.local_rank in [-1, 0]:
                    accuracy, eval_los = valid(
                        args, model, writer, test_loader, global_step, class2num
                    )
